...
class ApiGateway {
  ...
  execute = (config: IConfigRequest) => {
    const { method, resource, body, isFormDataType, params, queryParams, timeout } = config
    ...
    if (!configRequest?.baseURL || !configRequest?.url) {
      const error = ResponseModel.createError(400, '400', 'Invalid request configuration', 'Invalid request configuration', configRequest, configRequest)
    ...
    switch (method) {
      ...
      default:
        const error = ResponseModel.createError(405, '405', 'Unsupported HTTP method', 'Unsupported HTTP method', configRequest, configRequest)
        return Promise.resolve(error)
    }
  };
}
...
const parseFormData = (data: any, keepFormData?: boolean): FormData => {
